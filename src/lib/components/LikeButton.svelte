...
{#if $userId}
	<div class="comment-trigger">
		<div class="indicator">
			...
			{/if}
			<button class={`btn btn-${size}`} on:click={() => toggleLike(id)}>
				{#if $userLikes.includes(id)}
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 -960 960 960"
						class="w-6 h-6 fill-current translate-y-[1px] scale-90 lg:scale-100"
						><path
				...
				{:else}
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 -960 960 960"
						class="w-6 h-6 fill-black translate-y-[1px] scale-90 lg:scale-100"
						><path
